      ...
      <card-template>
        ...
        <template v-slot:card-content>
          <apex-chart
            type="bubble"
            height="300"
            :options="chartStockPriceBubble.options"
      ...
      <card-template>
        ...
        <template v-slot:card-content>
          <apex-chart
            type="treemap"
            height="300"
            :options="chartCategoryTreemap.options"
  ...
  computed: {
    ...
    chartStatus() {
      ...
      const counts = statuses.map(
        (st) => this.products.filter((p) => p.status === st).length,
      );
      return {
        options: {
          labels: statuses,
          colors: this.chartsColorsPalette.slice(0, statuses.length),
        },
        series: counts,
      };
    },
